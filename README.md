...
| `STREAM_URL_TTL` | `45m` | Local cache lifetime for resolved provider CDN URLs; TorBox refreshes its final URL after expiry or rejection |
...
| `ALLDEBRID_PROVIDER_COOLDOWN` | `1m` | Fallback provider-wide cooldown after an AllDebrid HTTP 503/429 without `Retry-After` |
| `SCRAPER_RATE_LIMIT_COOLDOWN` | `2s` | Cooldown for an addon after HTTP 429 when it does not provide `Retry-After` |
